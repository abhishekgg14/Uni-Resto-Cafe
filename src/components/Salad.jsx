...
const Salad = () => {
    ...
    const getData = () => {
        axios.post("https://run.mocky.io/v3/f47694b8-4d45-4c30-aed0-dd82bb4025fb", data).then(
            (response) => { const saladsAndSoupCategory = response.data.data[0].table_menu_list.find(category => category.menu_category === "Salads and Soup");
            if (saladsAndSoupCategory) {
                setData(saladsAndSoupCategory.category_dishes);
            } else {
                setData([]); // If the category is not found, set an empty array
            } }
        )
    }
    useEffect(() => { getData() }, [])
